# GPU deployment of the speech-to-speech realtime pipeline for HF Spaces (t4-small+):
# Parakeet STT -> Gemma 4 on Cerebras via the HF router -> native Keenable web
# search -> Qwen3 TTS (qwentts.cpp, CUDA). Based on the repo's own CUDA Dockerfile;
# Spaces route traffic to app_port (7860), the realtime websocket lives at /v1/realtime.
...
# Shell form so the HF_TOKEN / KEENABLE_API_KEY Space secrets expand at runtime.
